...
| `comment ls` | `rpc-backed (partial)` | **Initial paginated slice proven** via server-owned wrkq.comment.listView: --json, NDJSON/non-TTY, --porcelain first-page next_cursor (token byte-identical), limit+1, empty, AND cursor-replay no-dup/no-miss across pages (TestCommentLsCursorReplay). Cursor replay/no-dup-no-miss proven (TestCommentLsCursorReplay); malformed-cursor + sort (id/created_at/updated_at) + include-deleted parity. DELIBERATE DIVERGENCE: invalid --sort returns a clean validation error (legacy leaks a raw SQL "no such column" string). PENDING: yaml/tsv/table, multi-task + --reverse/desc (hard-gated). |
