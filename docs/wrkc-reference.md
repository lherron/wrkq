---
id: wrkq/wrkc-reference
title: wrkc collaboration reference
kind: reference
...
# wrkc collaboration reference

The injected agent guide (`wrkc info`, embedded at
`internal/rpccli/embedded/WRKC-USAGE.md`) carries the recipes an agent needs in
a session. This page carries the detail that guide deliberately leaves out:
...
acks commit together; omitting it keeps the wide rule above. A narrowed ack
records reason `reply_scoped`.
...
`--retry-after` arms a wrkq promise; at expiry the envelope returns to the
pending section of `wrkc inbox` and can then be acked by a reply. `wrkc show`
continues to report the envelope's stored `deferred` state together with its
`retryAt`; `inbox` is the surface that tells you what you currently owe.
...
on — and answering it is an ordinary say. `fyi` is never listed by `inbox`: it
carries no obligation and is acked at its own presentation.
...
A scope handle is `agent:<id>:project:<project>`, optionally followed by
`:task:<T-xxxxx>` or `:role:<role>`; no other trailing segment parses.
...
`failed:<reason>` and makes `--wait` exit non-zero. A wait that runs out of
`--timeout` exits non-zero with `monitor wait ended: timeout`, which is neither a
