...
// ──── monitor / watch bounded-polling read models (T-05115 / T-05116) ─────────
//
// Daedalus ruled (#10211) monitor + watch IN SCOPE as BOUNDED POLLING over RPC —
// NO server push/subscribe in v1. The server owns selector resolution,
// resource_id hydration, comment→task backfill, isEventIncluded /
// isStateChangeEvent, event-type filtering, the cursor high-water return, and the
// single authoritative condition snapshot. The CLIENT owns the blocking loop,
// timeout/stall clocks, exactly-ONE terminal NDJSON record, exit codes, and
// stdout/stderr. These read models read the generic event_log table (NOT
// workflow_events, which is the wrkf.event.query substrate).
...
func (a *API) MonitorEventsView(ctx context.Context, p MonitorEventsViewParams) (*WrkqMonitorEventsView, error) {
	...
	for rows.Next() {
		...
		// monitorRow and WrkqMonitorEvent share an identical field layout (the scan
		// row IS the wire row); a direct conversion keeps them in lockstep.
...
func (a *API) MonitorStateView(ctx context.Context, p MonitorStateViewParams) (*WrkqMonitorStateView, error) {
	...
	if evalErr != nil {
		// Legacy maps a missing watched task to exit code 3 (stream error). Surface
		// it as an internal error so the mirror exits 3.
...
// ──── selector resolution (server-owned) ───────────────────────────────────────
...
// ──── small helpers (server-local, ported from internal/cli) ───────────────────
