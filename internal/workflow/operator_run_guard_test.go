...
// task state unchanged. A reap-then-resolve positive control confirms the guard
// releases once the run is no longer active.
func TestOperatorTransitionRefusedWithActiveRun(t *testing.T) {
	...
	// Positive control: reap the run (reap-before-resolve), then the guard
	// releases and the operator transition commits.
	expireActionRunForTest(t, svc, implement.Binding.Run.ID)
	if _, err := svc.ReapActions(ReapActionsParams{InstanceID: inst.ID, ExpiredBefore: "2100-01-01T00:00:00Z"}); err != nil {
	...
	if err != nil {
		t.Fatalf("operator_resolved after reap: %v", err)
	}
