...
// TestWrkqTaskUpdate_NeedsSmoketestLabel proves the RPC update path
// (wrkq.task.update with patch.labels) reaches the same store update for the
// T-04891 needs_smoketest signal: creating with initial labels returns them,
// adding needs_smoketest while preserving existing labels persists the full
// replacement set, and show reflects it. The webhook label-edge SHAPE is
// asserted at the store layer (internal/store/tasks_webhooks_test.go); here we
// pin RPC label persistence/return shape.
func TestWrkqTaskUpdate_NeedsSmoketestLabel(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping subprocess in short mode")
	}
	dbPath := migratedDB(t)
	...
	frames := p2Run(t, dbPath,
		mkRPC("c1", "wrkq.task.create", map[string]any{
			"title":  "Smoketest Label Task",
			"kind":   "task",
			"labels": []string{"ui"},
		}),
	)
	created := p2ResultOrFail(t, frames[1], "wrkq.task.create with labels")
	if !labelsContain(created["labels"], "ui") {
		t.Fatalf("create labels = %#v, want ui", created["labels"])
	}
	if labelsContain(created["labels"], "needs_smoketest") {
		t.Fatalf("create labels = %#v, must not yet include needs_smoketest", created["labels"])
	}
	taskID, _ := created["id"].(string)
	etag, _ := created["etag"].(float64)
	...
	frames = p2Run(t, dbPath,
		mkRPC("u1", "wrkq.task.update", map[string]any{
			"task":       taskID,
			"patch":      map[string]any{"labels": []string{"ui", "needs_smoketest"}},
			"expectEtag": etag,
		}),
		mkRPC("s1", "wrkq.task.show", map[string]any{"task": taskID}),
	)
	updated := p2ResultOrFail(t, frames[1], "wrkq.task.update labels")
	if !labelsContain(updated["labels"], "needs_smoketest") {
		t.Fatalf("update labels = %#v, want needs_smoketest", updated["labels"])
	}
	if !labelsContain(updated["labels"], "ui") {
		t.Fatalf("update labels = %#v, must preserve existing ui label", updated["labels"])
	}
	shown := p2ResultOrFail(t, frames[2], "wrkq.task.show labels")
	if !labelsContain(shown["labels"], "needs_smoketest") || !labelsContain(shown["labels"], "ui") {
		t.Fatalf("show labels = %#v, want both ui and needs_smoketest", shown["labels"])
	}
}

// labelsContain reports whether a JSON-decoded labels value contains needle.
func labelsContain(raw any, needle string) bool {
	values, ok := raw.([]any)
	if !ok {
		return false
	}
