...
func TestWrkfLedgerAppendAndListOverRealRPC(t *testing.T) {
	...
	if got, _ := entry["writtenBy"].(string); got == "agent:forged" || !strings.HasPrefix(got, "agent:") {
		t.Fatalf("writtenBy = %#v, want a canonical engine-stamped caller (not forged input)", entry["writtenBy"])
	}
