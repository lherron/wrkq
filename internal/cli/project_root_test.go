...
// NOTE: the pure project-root transform (ApplyToPath/Selector/Paths) is proven
// once in internal/projectroot (TestTransform) — the shared neutral package both
// the legacy CLI and the mirror delegate to. The tests below cover the legacy
// CLI's INTEGRATION of that transform (runLs/findTasks + --project override),
// which is distinct from the unit transform.
