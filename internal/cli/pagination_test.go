...
func TestPaginationIntegration(t *testing.T) {
	...
	_, err = database.Exec(`
		INSERT INTO containers (uuid, id, slug, title, parent_uuid, kind, etag, created_at, updated_at, created_by_actor_uuid, updated_by_actor_uuid)
		VALUES ('container-uuid-1', 'P-00001', 'test-project', 'Test Project', (SELECT uuid FROM containers WHERE kind = 'root'), 'project', 1, datetime('now'), datetime('now'), 'actor-uuid-1', 'actor-uuid-1')
	`)
	if err != nil {
